...
if (data.status !== 200) {
  ...
  function showDetails(e, entry) {
    ...
    // append date and time
    const modalDate = entry.date_time_entry.slice(0, 10);
    const modalTime = entry.date_time_entry.slice(11, 16);

    const dateDOM = document.createElement("h2");
    dateDOM.className = "date";
    dateDOM.textContent = modalDate;
    leftSection.append(dateDOM);
    const timeDOM = document.createElement("h3");
    timeDOM.className = "time";
    timeDOM.textContent = modalTime;
    leftSection.append(timeDOM);

    // create the category
    const categoryInput = document.createElement("input");
    categoryInput.type = "text";
    categoryInput.defaultValue = entry.category;
    leftSection.append(categoryInput);
    //  append the details
